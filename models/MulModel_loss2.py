...
class ToyModel(torch.nn.Module):
    ...
    def forward(self, x1, edge_index1, x2, edge_index2, seeds):
        '''
        ...
        x1_seed, x2_seed = self.inter_propagate(x1, x2, seeds)
        x1 = self.conv3(x1, edge_index1, x2_seed)
        x2 = self.conv3(x2, edge_index2, x1_seed)
        return x1, x2

    def pre_forward(self, x1, edge_index1, x2, edge_index2):
        '''
        embeddings of source network g_s(V_s, E_s):
        x is node feature vectors of g_s, with shape [V_s, n_feats], V_s is the number of nodes,
            and n_feats is the dimension of features;
...
def get_embedding(s_x, t_x, s_e, t_e, g_s, g_t, edge3, anchor, test_edges, test_edges_false, dim=128, lr=0.001, alpha=0.5, \
        margin=0.8, neg=1, epochs=1000, verbose=2):
    ...
    s_input = s_x.shape[1]
    t_input = t_x.shape[1]
    model = ToyModel(s_input, t_input, dim)
    ...
    in_a, in_b, anchor_label = sample(anchor, g_s, g_t, neg=neg, seed=42) # hard negative sampling

    rw_loss1 = RWLoss(edge_index=edge3, walk_length=20, context_size=10, walks_per_node=10, p=1, q=1, num_nodes=s_x.shape[0] + t_x.shape[0]).to(device)
    loader1 = rw_loss1.loader(batch_size=512, shuffle=True, num_workers=1)
    # pw1, nw1 = next(iter(loader1))
    epoch = 0
    pre_epochs = 100
    while epoch < pre_epochs:
        for pw1, nw1 in loader1:
            model.train()
            optimizer.zero_grad()
            zs, zt = model.pre_forward(s_x, s_e, t_x, t_e)
            z = torch.vstack((zs, zt))
            loss = rw_loss1.forward(pw1, nw1, z, dim)
            loss.backward()
            optimizer.step()
            epoch += 1
            # scheduler.step()
            if verbose > 0 and epoch % 10 == 0:
                if verbose == 1:
                    ...
                    auc = evaluate(zt, test_edges, test_edges_false)
    ...
    while epoch < epochs:
        for pw1, nw1 in loader1:
            model.train()
            optimizer.zero_grad()
            zs, zt = model.forward(s_x, s_e, t_x, t_e, seeds)
            z = torch.vstack((zs, zt))
            intra_loss = rw_loss1.forward(pw1, nw1, z, dim)
            ...
            if verbose > 0 and epoch % 50 == 0:
                if verbose == 1:
                    ...
                    auc = evaluate(zt, test_edges, test_edges_false)
                    print('Epoch: {:03d}, intra_loss: {:.8f}, inter_loss: {:.8f}, loss_train: {:.8f}, auc: {:.8f}'.format(epoch + pre_epochs,\
                    intra_loss, inter_loss, loss, auc)) 
            if epoch >= epochs:
                break
